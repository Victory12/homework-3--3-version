# This Makefile is for the Local::MusicLibrary extension to perl.
#
...
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     EXE_FILES => [q[bin/music_library.pl]]
#     NAME => q[Local::MusicLibrary]
#     PREREQ_PM => {  }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Local/MusicLibrary.pm]
...
MAKE = make
FULLEXT = Local/MusicLibrary
BASEEXT = MusicLibrary
PARENT_NAME = Local
DLBASE = $(BASEEXT)
...
TO_INST_PM = lib/Local/MusicLibrary.pm \
	lib/Local/MusicLibraryParam.pm \
	lib/Local/Parser.pm
...
DISTNAME = Local-MusicLibrary
DISTVNAME = Local-MusicLibrary-1.00
...
pure_all :: $(INST_SCRIPT)/music_library.pl
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/music_library.pl 

$(INST_SCRIPT)/music_library.pl : bin/music_library.pl $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/music_library.pl
	$(CP) bin/music_library.pl $(INST_SCRIPT)/music_library.pl
	$(FIXIN) $(INST_SCRIPT)/music_library.pl
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/music_library.pl
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Local-MusicLibrary' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Local-MusicLibrary",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="Local-MusicLibrary" VERSION="1.00">' > Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.18" />' >> Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> Local-MusicLibrary.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> Local-MusicLibrary.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  ...
	  'lib/Local/MusicLibraryParam.pm' 'blib/lib/Local/MusicLibraryParam.pm' \
	  'lib/Local/Parser.pm' 'blib/lib/Local/Parser.pm' 
